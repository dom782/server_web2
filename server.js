...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', raw => {
    ...
    // Giacenza: web → PC
    if (t === 'giacenza_request') {
      if (!pcClient) {
        send(ws, { type: 'giacenza_response', request_id: msg.request_id,
          cod_art: msg.cod_art, error: 'PC non connesso', results: [] });
        return;
      }
      if (msg.request_id) {
    ...
    // Giacenza response: PC → web
    if (t === 'giacenza_response') {
      const origin = msg.request_id ? pendingSearch.get(msg.request_id) : null;
      if (origin && origin.readyState === 1) {
        send(origin, msg);
        pendingSearch.delete(msg.request_id);
      }
      return;
    }
